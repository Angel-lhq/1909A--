...
public class CustomAvaterView extends ViewGroup {
    ...
    public void initDatas(List<String> list) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            if (mIsLastComplete) {
                ...
                Glide.with(mContext).load(list.get(i)).apply(requestOptions).into(iv);
            } else {
                //GlideUtil.loadCircleImageView(mContext, list.get(size - i - 1), iv, mIsShowFrame, mFrameColor);
                RequestOptions requestOptions = new RequestOptions();
                requestOptions.circleCrop();
                Glide.with(mContext).load(list.get(size - i - 1)).apply(requestOptions).into(iv);
            }
    ...
    public void addData(String ava, AvaterListener listener1) {
        ...
        RequestOptions requestOptions = new RequestOptions();
        requestOptions.circleCrop();
        Glide.with(getContext()).load(ava).apply(requestOptions).into(iv);
        if (mIsLastComplete) {
            this.addView(iv);
        } else {
            this.addView(iv, 0);
